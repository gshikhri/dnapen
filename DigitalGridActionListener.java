...
public class DigitalGridActionListener implements ActionListener {
    ...
    static double TileHeight = 3; // Set default Tile Height
    static double TileWidth = 7; // Set default Tile Width
    static ArrayList<XYCoordinates> xyCoordinatesTileList = new ArrayList<XYCoordinates>();
    static DrawDataStack digitizedDataStack = new DrawDataStack();
    static boolean[][] gridValueAddedArray = new boolean[30][30];
    static boolean isTileDataSaved=false;
    static boolean isPDFSaved=false;
    static boolean isLatexSaved=false;
    static boolean isImageSaved=false;
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (!MainFrame.newTileCreated) {
            MainFrame.dnaTile = new DNABrick(TileHeight, TileWidth);
...
//            if(true){
    ...
    public class DrawCanvas extends Canvas {
        ...
        public DrawCanvas() {
            ...
            for(int i=0;i<15;i++)
            	for(int j=0;j<23;j++)
            MainFrame.selectedDigitized[i][j]=-1;
            ...
            digitizedDataStack.clearData();
                ...
                for(int i=0;i<xyCoordinatesTileList.size();i++){
                    gridValueAddedArray[xyCoordinatesTileList.get(i).getxCoordinate()][xyCoordinatesTileList.get(i).getyCoordinate()]=true;
                }
        ...
        public void paint(Graphics g) {
            ...
            */
                ...
                int yValue = 0;
                for (int i = 0; i < xyCoordinatesTileList.size(); i++) {
                    xValue = xyCoordinatesTileList.get(i).getxCoordinate();
                    yValue = xyCoordinatesTileList.get(i).getyCoordinate();
        ...
        public void fillElement(int xCoordinate, int yCoordinate) {
            ...
            if(z%2==1){
                ...
                xyCoordinatesTileList.add(new XYCoordinates(xCoordinate/ 90, yCoordinate / 30));
                digitizedDataStack.pushData(new XYCoordinates(xCoordinate/90, yCoordinate/30));
                MainFrame.selectedDigitized[xCoordinate/ 90][yCoordinate / 30]=MainFrame.digitizedTracker;
        ...
        public void removeElement(int xCoordinate, int yCoordinate) {
            ...
            if(z%2==1){
                ...
                MainFrame.selectedDigitized[xCoordinate/ 90][yCoordinate / 30]=-1;
                MainFrame.digitizedTracker--;
            }
            else {
                gridValueAddedArray[xCoordinate / 90][yCoordinate / 30] = false;
            ...
            for (int i = 0; i < xyCoordinatesTileList.size(); i++) {
                ...
                if(z%2==1){
                    if(xValue ==( xCoordinate / 90) && yValue == (yCoordinate / 30)){
                        xyCoordinatesTileList.remove(i);
                        MainFrame.selectedDigitized[xCoordinate/ 90][yCoordinate / 30]=-1;
                        MainFrame.digitizedTracker--;   
                    }
                }
        ...
        class MyMouseListener extends MouseAdapter implements MouseMotionListener {
            ...
            public void mousePressed(MouseEvent e) {
                MainFrame.isDigitizedSaved = false;
